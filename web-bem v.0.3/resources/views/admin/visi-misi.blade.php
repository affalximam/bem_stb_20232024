...
<body data-scroll-index="0" class="onloading">
    ...
    <section class="dashboard-admin pb-5">
        <div class="container">
            <div class="row">
                <h1 class="text-dark text-center">{{ $visi->nama_informasi }}</h1>
            </div>
            <form action="{{ route('admin.update_visi_misi') }}" method="POST">
                @csrf
                <div class="row mt-3">
                    <textarea name="visi" id="" class="textarea form-control">{{ $visi->isi_informasi }}</textarea>
                </div>
                <div class="row mt-3">
                    <h1 class="text-dark text-center">{{ $misi->nama_informasi }}</h1>
                </div>
                <div class="row mt-3">
                    <textarea name="misi" id="" class="textarea form-control">{{ $misi->isi_informasi }}</textarea>
